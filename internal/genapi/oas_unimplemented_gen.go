// Code generated by ogen, DO NOT EDIT.

package genapi

import (
	"context"
	"net/http"

	ht "github.com/ogen-go/ogen/http"
)

// UnimplementedHandler is no-op Handler which returns http.ErrNotImplemented.
type UnimplementedHandler struct{}

var _ Handler = UnimplementedHandler{}
var _ RawHandler = UnimplementedHandler{}

// ApplyEdit implements applyEdit operation.
//
// POST /documents/{id}/edits
func (UnimplementedHandler) ApplyEdit(ctx context.Context, req *BlockEdit, params ApplyEditParams) (r *Revision, _ error) {
	return r, ht.ErrNotImplemented
}

// CommitDocument implements commitDocument operation.
//
// POST /documents/{id}/commits
func (UnimplementedHandler) CommitDocument(ctx context.Context, params CommitDocumentParams) (r *Revision, _ error) {
	return r, ht.ErrNotImplemented
}

// CreateSession implements createSession operation.
//
// POST /sessions
func (UnimplementedHandler) CreateSession(ctx context.Context, req *CreateSessionRequest) (r *Session, _ error) {
	return r, ht.ErrNotImplemented
}

// GetBlocks implements getBlocks operation.
//
// GET /documents/{id}/blocks
func (UnimplementedHandler) GetBlocks(ctx context.Context, params GetBlocksParams) (r []Block, _ error) {
	return r, ht.ErrNotImplemented
}

// GetCandidates implements getCandidates operation.
//
// GET /documents/{id}/blocks/{bid}/candidates
func (UnimplementedHandler) GetCandidates(ctx context.Context, params GetCandidatesParams) (r []Candidate, _ error) {
	return r, ht.ErrNotImplemented
}

// GetDiff implements getDiff operation.
//
// GET /documents/{id}/diff
func (UnimplementedHandler) GetDiff(ctx context.Context, params GetDiffParams) (r []WordEdit, _ error) {
	return r, ht.ErrNotImplemented
}

// GetHealth implements getHealth operation.
//
// Liveness check.
//
// GET /health
func (UnimplementedHandler) GetHealth(ctx context.Context) (r *Health, _ error) {
	return r, ht.ErrNotImplemented
}

// GetHistory implements getHistory operation.
//
// GET /documents/{id}/history
func (UnimplementedHandler) GetHistory(ctx context.Context, params GetHistoryParams) (r []Revision, _ error) {
	return r, ht.ErrNotImplemented
}

// GetModelStatus implements getModelStatus operation.
//
// GET /models/{name}/status
func (UnimplementedHandler) GetModelStatus(ctx context.Context, params GetModelStatusParams) (r *LiveStateResponse, _ error) {
	return r, ht.ErrNotImplemented
}

// GetSessionMessages implements getSessionMessages operation.
//
// GET /sessions/{id}/messages
func (UnimplementedHandler) GetSessionMessages(ctx context.Context, params GetSessionMessagesParams) (r []Message, _ error) {
	return r, ht.ErrNotImplemented
}

// ListModels implements listModels operation.
//
// Discover the servable fleet (ADR-0018).
//
// GET /models
func (UnimplementedHandler) ListModels(ctx context.Context) (r []Model, _ error) {
	return r, ht.ErrNotImplemented
}

// ListModes implements listModes operation.
//
// GET /modes
func (UnimplementedHandler) ListModes(ctx context.Context) (r []Mode, _ error) {
	return r, ht.ErrNotImplemented
}

// ListSessions implements listSessions operation.
//
// GET /sessions
func (UnimplementedHandler) ListSessions(ctx context.Context, params ListSessionsParams) (r []Session, _ error) {
	return r, ht.ErrNotImplemented
}

// ListTools implements listTools operation.
//
// GET /tools
func (UnimplementedHandler) ListTools(ctx context.Context) (r []ToolDef, _ error) {
	return r, ht.ErrNotImplemented
}

// OpenDocument implements openDocument operation.
//
// Open/create a document by path; returns its surrogate id.
//
// POST /documents
func (UnimplementedHandler) OpenDocument(ctx context.Context, req *OpenDocumentRequest) (r *Document, _ error) {
	return r, ht.ErrNotImplemented
}

// ProvisionModel implements provisionModel operation.
//
// POST /models/{name}/provision
func (UnimplementedHandler) ProvisionModel(ctx context.Context, params ProvisionModelParams) (r *ProvisionResponse, _ error) {
	return r, ht.ErrNotImplemented
}

// StartModel implements startModel operation.
//
// POST /models/{name}/start
func (UnimplementedHandler) StartModel(ctx context.Context, params StartModelParams) (r *LiveStateResponse, _ error) {
	return r, ht.ErrNotImplemented
}

// StartTurn implements startTurn operation.
//
// Run one agent turn (SSE stream).
//
// POST /turn
func (UnimplementedHandler) StartTurn(ctx context.Context, req *Task, w http.ResponseWriter) error {
	return ht.ErrNotImplemented
}

// StopModel implements stopModel operation.
//
// POST /models/{name}/stop
func (UnimplementedHandler) StopModel(ctx context.Context, params StopModelParams) (r *LiveStateResponse, _ error) {
	return r, ht.ErrNotImplemented
}
